...
public class CalculateBottomCombinationsRenderer extends RendererBase {
    ...
    private final String title = "Calculate Bottom Combinations";
    ...
    public CalculateBottomCombinationsRenderer(ClueCell cell) {
        ...
        for (int index = 0; index < cell.getBottomCombinations().size(); ++index)
            contentList.add("Combination " + (index + 1) + ":   " + cell.getBottomCombinations().getCombination(index).toString());
